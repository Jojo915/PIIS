...
def run_chat_completion(
    ...
    if client is None:
        # No API key is configured yet (neither via the AI Settings panel
        # nor GEMINI_API_KEY), so there is no client to call. Degrade to
        # the same local fallback the real implementation below already
        # uses when the live API call itself fails, rather than raising --
        # indexing/enrichment should still work, just without AI text,
        # until the user saves a key.
